...
from azure_utils_mcp.client import get_servicebus_admin_client, get_servicebus_client
from azure_utils_mcp.tools.servicebus.utils import decode_body, decode_properties
...
def peek_subscription_messages(
    ...
    try:
        if session_id is None:
            props = get_servicebus_admin_client(namespace).get_subscription_runtime_properties(topic, subscription)
            if props.active_message_count == 0:
                return f"No messages found in subscription '{subscription}' on topic '{topic}'."
        client = get_servicebus_client(namespace)
